...
/**
 *  Test application for RDF filter. <blockquote><em>This application, both
 *  source code and documentation, is in the Public Domain, and comes with
 *  <strong>NO WARRANTY</strong> .</em></blockquote> <p>
 *
 *  This application will display RDF events to standard output, together with
 *  select XML events (start/end element and character-related events). It is
 *  intended for testing the functionality of RDFFilter with different RDF
 *  documents. It will also display the amount of memory used before and after
 *  each RDF document processing; that number goes to standard error rather than
 *  standard output.</p> <p>
 *
 *  This application will use the org.xml.sax.driver property, if set, to select
 *  a SAX2 XML reader. If that property is not set, the application will check
 *  the value of the org.xml.sax.parser property and wrap an XMLParserAdapter
 *  around it, using AElfred by default if no org.xml.sax.parser property is
 *  specified.</p> <p>
 *
 *  You must have SAX2beta2 and a SAX driver installed to use this test class.
 *  </p>
 *
...
public class TestRDFFilter extends DefaultHandler implements RDFHandler {
  ...
  /**
   *  Report a literal statement. <p>
   *
   *  The RDF filter will invoke this method for every statement in the RDF
   *  document that has a literal value. An RDF statement is, essentially, a
   *  single NAME=VALUE pair for an object of some sort.</p> <p>
   *
   *  Note that the subject may or may not be a proper URI; the subjectType
   *  property informs the application how to interpret it.</p> <p>
   *
   *  In this demonstration, the statement is simply printed to standard output,
   *  but without any information about the subject's type (normally it's not
   *  safe to ignore this). The object is place in quotation marks to show that
   *  it is a literal string.</p>
   *
   * @param  subjectType  The type of subject (from RDFHandler).
   * @param  subject      The subject of the statement (the identifier of the
   *      object being described).
   * @param  predicate    The predicate of the statement (the name of the
   *      object's property).
   * @param  object       The object of the statement (the value of the object's
   *      property), which is a literal string.
   * @param  lang         The language of the object (the property's value), as
   *      specified by an xml:lang attribute.
  ...
  /**
   *  Report a resource statement. <p>
   *
   *  The RDF filter will invoke this method for every statement in the RDF
   *  document that is a link to another resource. An RDF statement is,
   *  essentially, a single NAME=VALUE pair for an object of some sort.</p> <p>
   *
   *  Note that the subject may or may not be a proper URI; the subjectType
   *  property informs the application how to interpret it.</p> <p>
   *
   ...
   *      object's property).
   * @param  object       The object of the statement (the value of the object's
   *      property), which is the identifier of another object.
  ...
  /**
   *  Report the start of a literal XML statement. <p>
   *
   *  The RDF filter will invoke this method for every statement in the RDF
   *  document that has a literal XML value (i.e. rdf:parseType="Literal"). The
   *  statement is, essentially, a single NAME=VALUE pair for an object of some
   *  sort.</p> <p>
   *
   *  Note that the subject may or may not be a proper URI; the subjectType
   *  property informs the application how to interpret it.</p> <p>
   *
   *  In this demonstration, the start message is simply printed to standard
   *  output, but without any information about the subject's type (normally
   *  it's not safe to ignore this). The XML markup inside the property is
   *  reported through regular SAX2 events, followed by an endXMLStatement
   *  event.</p>
   *
   * @param  subjectType  The type of subject (from RDFHandler).
   * @param  subject      The subject of the statement (the identifier of the
  ...
  /**
   *  Report the end of a literal XML statement. <p>
   *
   *  This method is invoked after the XML content of a statement has been
   *  reported through regular SAX2 callbacks.</p>
   *
...
//	System.out.println("{[end xml]}");
  ...
  /**
   *  Show the start of an XML element. <p>
   *
   *  For now, show only the local name and no attributes.</p>
   *
  ...
  /**
   *  Show the end of an XML element. <p>
   *
   *  For now, show only the local name.</p>
   *
   * @param  uri        The Namespace URI.
   * @param  localName  The element's local name.
   * @param  rawName    The element's raw name.
...
//	System.out.print("\n");
   ...
   *  Main entry point. <p>
   *
   *  Process all XML documents supplied on the command line (local file names,
   *  not URIs), and report the events.</p>
   *
